...
############### Noise
set.seed(2)
E1 <- t(matrix(rnorm(n*k, mean=0, sd=0.03),nrow=24))### ### HERE TO MALIPULATE X1 ERROR
set.seed(3)
E2 <- t(matrix(rnorm(n*k, mean=0, sd=0.03),nrow=24)) ### ### HERE TO MALIPULATE X2 ERROR
...
pcajoint1 <- pca(t(Results[["joint"]][[1]]), ncomp = 1) 
pcajoint2 <- pca(t(Results[["joint"]][[2]]), ncomp = 1) 

plot(S01,  pcajoint1[["variates"]][["X"]][,1] ,  main= "X1 scores", ylab = "JIVE Individual scores pc1", xlab = "Individual variation scores")
plot(S01,  pcajoint2[["variates"]][["X"]][,1] ,  main= "X2 scores", ylab = "JIVE Individual scores pc1", xlab = "Individual variation scores")

plot(u11, pcajoint1[["loadings"]][["X"]][,1] ,  main= "X1 loadings", ylab = "JIVE Individual loadings pc2", xlab = "Joint variation loadings")
plot(u21, pcajoint2[["loadings"]][["X"]][,1] ,  main= "X2 loadings", ylab = "JIVE Individual loadings pc2", xlab = "Joint variation loadings")


#temp <-as.numeric( pcajoint[["variates"]][["X"]] )
#plot(temp, S01)
#plot(u11, pcajoint[["loadings"]][["X"]] )
...
############## # results and plots
Noise <- c(85,70,45,20, 5)
Ser <- c(0.57,0.084,0.038, 0.1 , 0.7 )*5
Ler <- c(0.6 ,0.57 ,0.65,  0.60, 0.66)
...
Noise <- c(85,70,45,20, 5)
Ser <- c(0.72,0.084,0.0034, 0.1 , 0.43 )*5
Ler <- c(0.57 ,0.57 ,0.57,  0.60, 0.91)
...
ggplot(a, aes(x=Noise)) + 
  ...
  geom_line(aes(y = Ler), color="steelblue", lwd=1.5) + ggtitle("Error per persentage of Joint variation explained") +
  xlab("Joint variation percentage") + ylab("Error") # + ylim(0, 2) + xlim(0, 70)
